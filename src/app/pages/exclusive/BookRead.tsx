...
const BookReadPage: FC<React.PropsWithChildren<unknown>> = () => {
  ...
  const changeRange = (e: any) => {
    ...
    if (pageNum < 1) setPageNumber(1)
    else setPageNumber(pageNum)
  ...
  let filePDF = ''
  if (extra_file?.value_extra) filePDF = extra_file?.value_extra
  ...
  return (
    <>
      ...
      <br />
      <div className='row'>
        <div className='col-3 col-md-1'>Progresso</div>
          ...
          >
            {({size}) => (
              <div style={{height: 'auto'}}>
                <div className='row '>
                  <div className='col-1 d-none d-lg-block'>Zoom</div>
...
const BookRead: FC<React.PropsWithChildren<unknown>> = () => {
  const intl = useIntl()
  const component = useSelector((state: ApplicationState) => state.component)
  ...
  return (
    <>
      <PageTitle breadcrumbs={[]}>
        {intl.formatMessage({id: 'MENU.BOOKREAD'}) + ' - ' + component.data.name}
      </PageTitle>
