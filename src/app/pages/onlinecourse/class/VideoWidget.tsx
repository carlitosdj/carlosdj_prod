...
import { createAulaConcluidaRequest, createRateRequest } from '../../../../store/ducks/component/actions'
...
const StarRating = (selectedClass: any) => {
  ...
  useEffect(() => {
    //console.log("Bateu")
    setRating(selectedClass.selectedClass.aulaconcluida[0]?.rate)
  }, [selectedClass.selectedClass.id])

  const rate = (index:number) => {
    setRating(index)
    dispatch(createRateRequest(selectedClass.selectedClass.aulaconcluida[0]?.id, me.me.id!, selectedClass.selectedClass.id, index))
